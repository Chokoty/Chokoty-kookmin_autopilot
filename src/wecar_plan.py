...
class wecar_planner():
    ...
    def __init__(self):
        ...
        self.lattice_current_lane=3


    def wecar_running(self):
        ...
        if self.is_status==True  and self.is_obj==True: ## 차량의 상태, 장애물 상태 점검
            ...
            lattice_path,selected_lane=latticePlanner(local_path,global_obj,vehicle_status,self.lattice_current_lane)
            self.lattice_current_lane=selected_lane
            ...
            self.steering=self.pure_pursuit.steering_angle()
                
            self.cc_vel = self.cc.acc(local_obj,self.status_msg.velocity.x,self.vel_profile[self.current_waypoint],self.status_msg) ## advanced cruise control 적용한 속도 계획
        ...
        if self.count==300 : ## global path 출력
